...
import { Typography, Dropdown, Space, Popover, Divider, Splitter } from "antd";
...
export default function TextArea({
  ...
  const [startRephrasing, setStartRephrasing] = useState(false);
  ...
  const rephraseSelectedText = async () => {
    ...
    setPopup(false);
    setStartRephrasing(true);
    const index = content.indexOf(selectedText);
    setSelectIndex({ start: index, end: index + selectedText.length });
    try {
  ...
  return (
    <>
      {loading && (
        <div className="absolute top-0 left-0 w-full h-full bg-black opacity-30 flex items-center justify-center z-100">
          <div className="animate-spin rounded-full h-8 w-8 border-4 border-t-transparent border-white" />
        </div>
      )}
      <Splitter
        collapsibleIcon={{
          start: <CaretLeftOutlined className="text-lg" />,
          end: <CaretRightOutlined className="text-lg" />,
        }}
      >
        {(mode === "add" || isEdit) && (
          <Splitter.Panel
        ...
        <Splitter.Panel style={{ padding: "20px" }}>
          {mode === "add" && <Title level={3}>创建新文章</Title>}
          <div className="relative flex flex-col gap-4 bg-white p-4 mt-3 rounded">
            {mode === "add" || editMode ? (
              ...
              <>
                ...
                {startRephrasing && (
                  <div style={{ marginTop: "20px" }}>
                    ...
                    >
                      ....
                      {content.slice(selectIndex.start - 10, selectIndex.start)}
                      <span style={{ background: "#FFEA00" }}>
                        {generatedContent ? (
                          `${generatedContent}`
                        ) : (
                      ...
                      {content.slice(selectIndex.end, selectIndex.end + 10)}
                      .....
                    </div>
                    <div>
                      <button
                      ...
                      <button
                        ...
                        onClick={() => {
                          setContent(
                            content.replace(selectedText, generatedContent)
                          );
                          setSelectedText("");
                          setStartRephrasing(false);
                          setGeneratedContent("");
